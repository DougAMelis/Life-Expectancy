...
I created a model named baseline with all regressors. This baseline will be used to compare to the models with the best predictors.
For each of the models, I calculated the PRESS statistic, and variance inflation factors for the model. Then I tested for normality.
For Model 1, I used stepwise regression to select the regressors.
```{r}
baseline <- lm(Life.Expectancy ~ ., data.train)
stepwise <- ols_step_both_p(baseline, pent=.04, prem=.05)
stepwise
model.1 <- lm(Life.Expectancy ~ GDP.ln + Immunization + HIV.Prevalence + BMI.abs + BMI + Diabetes + Alcohol + GDP, data.train)
summary(model.1)
...
I performed cross validation on my models.
```{r}
...
cvFits <- cvSelect(Baseline = cvfit0, Model1 = cvfit1, Model2 = cvfit2)
cvFits
```
